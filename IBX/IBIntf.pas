...
function isc_release_savepoint_stub(status_vector : PISC_STATUS;
                                   tran_handle :  PISC_TR_HANDLE;
                                   tran_name : PAnsiChar) : ISC_STATUS; stdcall;
begin
...
function isc_rollback_savepoint_stub(status_vector : PISC_STATUS; tran_handle : PISC_TR_HANDLE;
   tran_name : PAnsiChar; Option : UShort) :  ISC_STATUS; stdcall;
begin
...
var
  ...
  function TryGetProcAddr(ProcName: AnsiString): Pointer;
  begin
    Result := GetProcAddress(CurLibrary, PAnsiChar(ProcName));
  end;

  function GetProcAddr(ProcName: AnsiString): Pointer;
  begin
    Result := GetProcAddress(CurLibrary, PAnsiChar(ProcName));
...
begin
  ...
  begin
    ...
    IBXMLLibrary := LoadLibrary(IBXML_DLL);
...
procedure LoadIBInstallLibrary;
  ...
  function GetProcAddr(ProcName: AnsiString): Pointer;
  begin
    Result := GetProcAddress(IBInstallLibrary, PAnsiChar(ProcName));
    if not Assigned(Result) then
      RaiseLastOSError;
  end;
...
function TDynamicLibrary.BLOB_put(isc_arg1: AnsiChar; isc_arg2: PBSTREAM): Int;
begin
...
function TDynamicLibrary.isc_dsql_batch_execute_immed(status_vector : PISC_STATUS;
						   db_handle : PISC_DB_HANDLE; tran_handle : PISC_TR_HANDLE;
						   Dialect : UShort; no_of_sql : ULong; statement : PPByte;
						   rows_affected : PULong): ISC_STATUS;
begin
...
function TDynamicLibrary.isc_event_block(event_buffer : PPAnsiChar; result_buffer : PPAnsiChar;
				                     id_count : UShort; event_list : array of PAnsiChar): ISC_LONG;
begin
...
function TDynamicLibrary.isc_free(isc_arg1: PAnsiChar): ISC_LONG;
begin
...
procedure TDynamicLibrary.isc_get_client_version(buffer: PByte);
begin
...
function TDynamicLibrary.isc_interprete(buffer: PByte;
...
procedure TDynamicLibrary.isc_sql_interprete(sqlcode: Short; buffer: PByte;
  buffer_length: Short);
begin
...
function TDynamicLibrary.isc_vax_integer(buffer: PByte;
