...
uses
  ...
  TaskDialog, FrontLog_Unit, Grids, BaseGrid, AdvGrid, DBAdvGrid, AdvObj;
...
type
  ...
  TRestMainForm = class(TBaseFrontForm)
    ...
    procedure DBAdvGrMainFooterCalc(Sender: TObject; ACol, ARow: Integer;
      var Value: String);
    procedure FrontTimerTimer(Sender: TObject);
  private
...
//  FFrontLog.FrontBase := FFrontBase;
  ...
  with DBAdvGrMain do
  begin
    Columns[1].HTMLTemplate := '<#GOODNAME><BR><FONT color="clBlue"><#MODIFYSTRING></FONT>';
    DefaultRowHeight := 4 * cn_FontSize;
...
begin
  ...
  FLineTable.FieldDefs.Add('usr$quantity', ftFloat, 0);
  FLineTable.FieldDefs.Add('usr$costncu', ftFloat, 0);
  FLineTable.FieldDefs.Add('usr$goodkey', ftInteger, 0);
  FLineTable.FieldDefs.Add('usr$sumncuwithdiscount', ftFloat, 0);
  FLineTable.FieldDefs.Add('usr$sumncu', ftFloat, 0);
  FLineTable.FieldDefs.Add('usr$costncuwithdiscount', ftFloat, 0);
  FLineTable.FieldDefs.Add('usr$sumdiscount', ftFloat, 0);
  FLineTable.FieldDefs.Add('usr$persdiscount', ftFloat, 0);
  FLineTable.FieldDefs.Add('usr$causedeletekey', ftInteger, 0);
  FLineTable.FieldDefs.Add('usr$deleteamount', ftFloat, 0);
  ...
  FLineTable.FieldDefs.Add('oldquantity', ftFloat, 0);
...
begin
  ...
  begin
    FLineTable.Append;
    if FLineTable.State = dsBrowse then
      FLineTable.Append;
...
procedure TRestMainForm.FrontTimerTimer(Sender: TObject);
begin
  FormState := Pass;
  FrontTimer.Enabled := False;
end;
